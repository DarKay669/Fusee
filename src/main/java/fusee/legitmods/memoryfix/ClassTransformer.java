...
public class ClassTransformer implements IClassTransformer
{
    ...
    { 
        ...
        if (name.equals("fusee.legitmods.memoryfix.CapeImageBuffer"))
        {
    ...
    private byte[] transformCapeUtils(byte[] bytes)
    {
        ...
        RemappingClassAdapter adapter = new RemappingClassAdapter(classWriter, new Remapper()
        {
            public String map(String typeName)
            {
                ...
                {
                    return "fusee.legitmods.memoryfix.CapeImageBuffer".replace('.', '/');
                }
    ...
    private void transformAbstractResourcePack(ClassNode clazz, MethodNode method)
    {
        if ((method.name.equals("getPackImage") || method.name.equals("func_110586_a")) && method.desc.equals("()Ljava/awt/image/BufferedImage;"))
        {
            ...
            while (iter.hasNext())
            {
                ...
                {
                    method.instructions.insertBefore(insn, (AbstractInsnNode) new MethodInsnNode(184, "fusee.legitmods.memoryfix.ResourcePackImageScaler".replace('.', '/'), "scalePackImage", "(Ljava/awt/image/BufferedImage;)Ljava/awt/image/BufferedImage;", false));
                }
            }
        }
    }
